//-------------------------------------------------------------------------------------------------------
// VST Plug-Ins SDK
// Version 2.4		$Date: 2006/02/02 15:04:07 $
//
...
static pascal OSStatus windowHandler(
...
bool checkEffectEditor(AEffect* effect) {
    ...
    DialogBoxIndirectParam(
        GetModuleHandle(0), &t, 0, (DLGPROC)EditorProc, (LPARAM)effect);
    ...
    InstallWindowEventHandler(
        window, windowHandler, GetEventTypeCount(eventTypes), eventTypes, window, NULL);
    ...
    InstallEventLoopTimer(GetCurrentEventLoop(), kEventDurationSecond / 25.,
        kEventDurationSecond / 25., idleTimerProc, effect, &idleEventLoopTimer);
...
INT_PTR CALLBACK EditorProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (msg) {
        ...
        case WM_TIMER:
            if (effect) effect->dispatcher(effect, effEditIdle, 0, 0, 0, 0);
            break;
        ...
        case WM_CLOSE: {
            ...
            printf("HOST> Close editor..\n");
            if (effect) effect->dispatcher(effect, effEditClose, 0, 0, 0, 0);
