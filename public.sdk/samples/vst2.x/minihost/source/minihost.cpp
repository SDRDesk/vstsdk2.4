...
struct PluginLoader {
    ...
    bool loadLibrary(const char* fileName) {
        ...
        CFURLRef url = CFURLCreateWithFileSystemPath(
            NULL, fileNameString, kCFURLPOSIXPathStyle, false);
        CFRelease(fileNameString);
        if (url == 0) return false;
        module = CFBundleCreate(NULL, url);
        CFRelease(url);
        if (module && CFBundleLoadExecutable((CFBundleRef)module) == false) return false;
#endif
        return module != 0;
    }
...
static bool checkPlatform() {
    ...
    printf("VstIntPtr = %d Bytes, VstInt32 = %d Bytes, Pointer = %d Bytes, AEffect = %d "
           "Bytes\n\n",
...
void checkEffectProcessing(AEffect* effect) {
    ...
    if (numInputs > 0) {
        for (VstInt32 i = 0; i < numInputs; i++) delete[] inputs[i];
        delete[] inputs;
    }

    if (numOutputs > 0) {
        for (VstInt32 i = 0; i < numOutputs; i++) delete[] outputs[i];
        delete[] outputs;
    }
}
