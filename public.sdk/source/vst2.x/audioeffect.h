...
class AudioEffect {
    ...
    AudioEffect(audioMasterCallback audioMaster, VstInt32 numPrograms,
        VstInt32 numParams); ///< Create an \e AudioEffect object
    ...
    virtual VstIntPtr dispatcher(VstInt32 opcode, VstInt32 index, VstIntPtr value,
        void* ptr, float opt); ///< Opcodes dispatcher
...
#pragma warning(disable : 4458)
    ...
    } ///< Called when the Maximun block size changes (always in a suspend state). Note
      ///< that the sampleFrames in Process Calls could be smaller than this block size,
      ///< but NOT bigger.
#pragma warning(default : 4458)
    virtual void processReplacing(float** inputs, float** outputs, VstInt32 sampleFrames)
        = 0; ///< Process 32 bit (single precision) floats (always in a resume state)

#if VST_2_4_EXTENSIONS
    virtual void processDoubleReplacing(double** inputs, double** outputs,
        VstInt32 sampleFrames) {} ///< Process 64 bit (double precision) floats (always in
                                  ///< a resume state) \sa processReplacing
    ...
    virtual float getParameter(VstInt32 index) {
        return 0;
    } ///< Return the value of the parameter with \e index
    virtual void setParameterAutomated(VstInt32 index,
        float value); ///< Called after a control has changed in the editor and when the
                      ///< associated parameter should be automated
    //@}
    ...
    virtual void setProgram(VstInt32 program) {
        curProgram = program;
    } ///< Set the current program to \e program

    virtual void setProgramName(char* name) {
    } ///< Stuff the name field of the current program with \e name. Limited to
      ///< #kVstMaxProgNameLen.
    virtual void getProgramName(char* name) {
        *name = 0;
    } ///< Stuff \e name with the name of the current program. Limited to
      ///< #kVstMaxProgNameLen.

    virtual void getParameterLabel(VstInt32 index, char* label) {
        *label = 0;
    } ///< Stuff \e label with the units in which parameter \e index is displayed (i.e.
      ///< "sec", "dB", "type", etc...). Limited to #kVstMaxParamStrLen.
    virtual void getParameterDisplay(VstInt32 index, char* text) {
        *text = 0;
    } ///< Stuff \e text with a string representation ("0.5", "-3", "PLATE", etc...) of
      ///< the value of parameter \e index. Limited to #kVstMaxParamStrLen.
    virtual void getParameterName(VstInt32 index, char* text) {
        *text = 0;
    } ///< Stuff \e text with the name ("Time", "Gain", "RoomType", etc...) of parameter
      ///< \e index. Limited to #kVstMaxParamStrLen.
    ...
    } ///< Set the number of inputs the plug-in will handle. For a plug-in which could
      ///< change its IO configuration, this number is the maximun available inputs.
    virtual void setNumOutputs(VstInt32 outputs) {
        cEffect.numOutputs = outputs;
    } ///< Set the number of outputs the plug-in will handle. For a plug-in which could
      ///< change its IO configuration, this number is the maximun available ouputs.

    virtual void canProcessReplacing(bool state
        = true); ///< Tells that processReplacing() could be used. Mandatory in VST 2.4!

#if VST_2_4_EXTENSIONS
    virtual void canDoubleReplacing(
        bool state = true); ///< Tells that processDoubleReplacing() is implemented.
#endif // VST_2_4_EXTENSIONS

    virtual void programsAreChunks(
        bool state = true); ///< Program data is handled in formatless chunks (using
                            ///< getChunk-setChunks)
    virtual void setInitialDelay(
    ...
    //@{
    void setEditor(
        AEffEditor* editor); ///< Should be called if you want to define your own editor
    ...
    //@{
    virtual VstInt32
    getMasterVersion(); ///< Returns the Host's version (for example 2400 for VST 2.4)
    virtual VstInt32 getCurrentUniqueId(); ///< Returns current unique identifier when
                                           ///< loading shell plug-ins
    virtual void masterIdle(); ///< Give idle time to Host application
    virtual bool isInputConnected(
        VstInt32 input); ///< Returns the input's connection state
    virtual bool isOutputConnected(
        VstInt32 output); ///< Returns the output's connection state
    //@}
    ...
    virtual void dB2string(float value, char* text,
        VstInt32 maxLen); ///< Stuffs \e text with an amplitude on the [0.0, 1.0] scale
                          ///< converted to its value in decibels.
    virtual void Hz2string(float samples, char* text,
        VstInt32 maxLen); ///< Stuffs \e text with the frequency in Hertz that has a
                          ///< period of \e samples.
    virtual void ms2string(float samples, char* text,
        VstInt32 maxLen); ///< Stuffs \e text with the duration in milliseconds of \e
                          ///< samples frames.
    virtual void float2string(float value, char* text,
        VstInt32 maxLen); ///< Stuffs \e text with a string representation on the floating
                          ///< point \e value.
    virtual void int2string(VstInt32 value, char* text,
        VstInt32 maxLen); ///< Stuffs \e text with a string representation on the integer
                          ///< \e value.
    //@}
    ...
    //-------------------------------------------------------------------------------------------------------
    /// @cond ignore
    virtual void DECLARE_VST_DEPRECATED(process)(
