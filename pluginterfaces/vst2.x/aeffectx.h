//-------------------------------------------------------------------------------------------------------
// VST Plug-Ins SDK
// Version 2.4		$Date: 2006/02/13 12:04:43 $
//
...
enum Vst2StringConstants {
    //-------------------------------------------------------------------------------------------------------
    kVstMaxNameLen = 64, ///< used for #MidiProgramName, #MidiProgramCategory,
                         ///< #MidiKeyName, #VstSpeakerProperties, #VstPinProperties
    kVstMaxLabelLen
    = 64, ///< used for #VstParameterProperties->label, #VstPinProperties->label
    kVstMaxShortLabelLen
    = 8, ///< used for #VstParameterProperties->shortLabel, #VstPinProperties->shortLabel
...
struct VstEvent {
    ...
    VstInt32
        deltaFrames; ///< sample frames related to the current block start sample position
...
struct VstMidiEvent {
    ...
    VstInt32 noteOffset; ///< offset (in sample frames) into note from note start if
                         ///< available, else 0
    char midiData[4]; ///< 1 to 3 MIDI bytes; midiData[3] is reserved (zero)
    char detune; ///< -64 to +63 cents; for scales other than 'well-tempered'
                 ///< ('microtuning')
...
enum VstMidiEventFlags {
    ...
    kVstMidiEventIsRealtime = 1
        << 0 ///< means that this event is played life (not in playback from a sequencer
             ///< track).\n This allows the Plug-In to handle these flagged events with
             ///< higher priority, especially when the Plug-In has a big latency
...
\note VstTimeInfo::samplePos :Current Position. It must always be valid, and should not
cost a lot to ask for. The sample position is ahead of the time displayed to the user. In
sequencer stop mode, its value does not change. A 32 bit integer is too small for sample
positions, and it's a double to make it easier to convert between ppq and samples. \note
VstTimeInfo::ppqPos : At tempo 120, 1 quarter makes 1/2 second, so 2.0 ppq translates to
48000 samples at 48kHz sample rate. .25 ppq is one sixteenth note then. if you need
something like 480ppq, you simply multiply ppq by that scaler. \note
VstTimeInfo::barStartPos : Say we're at bars/beats readout 3.3.3. That's 2 bars + 2 q + 2
...
VstTimeInfo::samplesToNextClock : MIDI Clock Resolution (24 per Quarter Note), can be
negative the distance to the next midi clock (24 ppq, pulses per quarter) in samples.
unless samplePos falls precicely on a midi clock, this will either be negative such that
the previous MIDI clock is addressed, or positive when referencing the following (future)
MIDI clock.
*/
//-------------------------------------------------------------------------------------------------------
struct VstTimeInfo {
    ...
    VstInt32 smpteOffset; ///< SMPTE offset (in SMPTE subframes (bits; 1/80 of a frame)).
    ...
    VstInt32 samplesToNextClock; ///< MIDI Clock Resolution (24 Per Quarter Note), can be
                                 ///< negative (nearest clock)
...
enum VstTimeInfoFlags {
    ...
    = 1 << 6, ///< set if automation write mode active (record parameter changes)
    kVstAutomationReading
    = 1 << 7, ///< set if automation read mode active (play parameter changes)
    ...
    kVstCyclePosValid
    = 1 << 12, ///< VstTimeInfo::cycleStartPos and VstTimeInfo::cycleEndPos valid
    kVstTimeSigValid = 1 << 13, ///< VstTimeInfo::timeSigNumerator and
                                ///< VstTimeInfo::timeSigDenominator valid
    kVstSmpteValid
    = 1 << 14, ///< VstTimeInfo::smpteOffset and VstTimeInfo::smpteFrameRate valid
...
struct VstVariableIo {
    ...
    VstInt32*
        numSamplesOutputProcessed; ///< number of samples actually processed of output
    //-------------------------------------------------------------------------------------------------------
};
...
enum AudioMasterOpcodesX {
    ...
    = audioMasterPinConnected + 2, ///< \deprecated deprecated in VST 2.4

    audioMasterGetTime, ///< [return value]: #VstTimeInfo* or null if not supported
                        ///< [value]: request mask  @see VstTimeInfoFlags @see
                        ///< AudioEffectX::getTimeInfo
    audioMasterProcessEvents, ///< [ptr]: pointer to #VstEvents  @see VstEvents @see
    ...
    audioMasterIOChanged, ///< [return value]: 1 if supported  @see
                          ///< AudioEffectX::ioChanged

    DECLARE_VST_DEPRECATED(audioMasterNeedIdle), ///< \deprecated deprecated in VST 2.4

    audioMasterSizeWindow, ///< [index]: new width [value]: new height [return value]: 1
                           ///< if supported  @see AudioEffectX::sizeWindow
    audioMasterGetSampleRate, ///< [return value]: current sample rate  @see
                              ///< AudioEffectX::updateSampleRate
    audioMasterGetBlockSize, ///< [return value]: current block size  @see
                             ///< AudioEffectX::updateBlockSize
    audioMasterGetInputLatency, ///< [return value]: input latency in audio samples  @see
                                ///< AudioEffectX::getInputLatency
    audioMasterGetOutputLatency, ///< [return value]: output latency in audio samples @see
    ...
    audioMasterGetCurrentProcessLevel, ///< [return value]: current process level  @see
                                       ///< VstProcessLevels
    audioMasterGetAutomationState, ///< [return value]: current automation state  @see
                                   ///< VstAutomationStates

    audioMasterOfflineStart, ///< [index]: numNewAudioFiles [value]: numAudioFiles [ptr]:
                             ///< #VstAudioFile*  @see AudioEffectX::offlineStart
    audioMasterOfflineRead, ///< [index]: bool readSource [value]: #VstOfflineOption* @see
                            ///< VstOfflineOption [ptr]: #VstOfflineTask*  @see
                            ///< VstOfflineTask @see AudioEffectX::offlineRead
    audioMasterOfflineWrite, ///< @see audioMasterOfflineRead @see
                             ///< AudioEffectX::offlineRead
    audioMasterOfflineGetCurrentPass, ///< @see AudioEffectX::offlineGetCurrentPass
    audioMasterOfflineGetCurrentMetaPass, ///< @see
    ...
    audioMasterGetVendorString, ///< [ptr]: char buffer for vendor string, limited to
                                ///< #kVstMaxVendorStrLen  @see
                                ///< AudioEffectX::getHostVendorString
    audioMasterGetProductString, ///< [ptr]: char buffer for vendor string, limited to
                                 ///< #kVstMaxProductStrLen  @see
                                 ///< AudioEffectX::getHostProductString
    audioMasterGetVendorVersion, ///< [return value]: vendor-specific version  @see
                                 ///< AudioEffectX::getHostVendorVersion
    audioMasterVendorSpecific, ///< no definition, vendor specific handling  @see
    ...
    audioMasterGetDirectory, ///< [return value]: FSSpec on MAC, else char*  @see
    ...
    audioMasterCloseFileSelector, ///< [ptr]: VstFileSelect*  @see
                                  ///< AudioEffectX::closeFileSelector

    DECLARE_VST_DEPRECATED(audioMasterEditFile), ///< \deprecated deprecated in VST 2.4

    DECLARE_VST_DEPRECATED(
        audioMasterGetChunkFile), ///< \deprecated deprecated in VST 2.4 [ptr]: char[2048]
                                  ///< or sizeof(FSSpec) [return value]: 1 if supported
                                  ///< @see AudioEffectX::getChunkFile
...
enum AEffectXOpcodes {
    //-------------------------------------------------------------------------------------------------------
    effProcessEvents
    = effSetChunk + 1 ///< [ptr]: #VstEvents*  @see AudioEffectX::processEvents

    ,
    effCanBeAutomated ///< [index]: parameter index [return value]: 1=true, 0=false  @see
                      ///< AudioEffectX::canParameterBeAutomated
    ,
    effString2Parameter ///< [index]: parameter index [ptr]: parameter string [return
                        ///< value]: true for success  @see AudioEffectX::string2parameter
    ...
    ,
    effGetProgramNameIndexed ///< [index]: program index [ptr]: buffer for program name,
                             ///< limited to #kVstMaxProgNameLen [return value]: true for
                             ///< success  @see AudioEffectX::getProgramNameIndexed
    ...
    ,
    effGetInputProperties ///< [index]: input index [ptr]: #VstPinProperties* [return
                          ///< value]: 1 if supported  @see
                          ///< AudioEffectX::getInputProperties
    ,
    effGetOutputProperties ///< [index]: output index [ptr]: #VstPinProperties* [return
                           ///< value]: 1 if supported  @see
                           ///< AudioEffectX::getOutputProperties
    ,
    effGetPlugCategory ///< [return value]: category  @see VstPlugCategory @see
    ...
    ,
    effOfflinePrepare ///< [ptr]: #VstOfflineTask array [value]: count  @see
                      ///< AudioEffectX::offlinePrepare
    ,
    effOfflineRun ///< [ptr]: #VstOfflineTask array [value]: count  @see
                  ///< AudioEffectX::offlineRun

    ,
    effProcessVarIo ///< [ptr]: #VstVariableIo*  @see AudioEffectX::processVariableIo
    ,
    effSetSpeakerArrangement ///< [value]: input #VstSpeakerArrangement* [ptr]: output
                             ///< #VstSpeakerArrangement*  @see
    ...
    ,
    effGetVendorString ///< [ptr]: buffer for effect vendor string, limited to
                       ///< #kVstMaxVendorStrLen  @see AudioEffectX::getVendorString
    ,
    effGetProductString ///< [ptr]: buffer for effect vendor string, limited to
                        ///< #kVstMaxProductStrLen  @see AudioEffectX::getProductString
    ,
    effGetVendorVersion ///< [return value]: vendor-specific version  @see
                        ///< AudioEffectX::getVendorVersion
    ,
    effVendorSpecific ///< no definition, vendor specific handling  @see
                      ///< AudioEffectX::vendorSpecific
    ,
    effCanDo ///< [ptr]: "can do" string [return value]: 0: "don't know" -1: "no" 1: "yes"
             ///< @see AudioEffectX::canDo
    ,
    effGetTailSize ///< [return value]: tail size (for example the reverb time of a reverb
                   ///< plug-in); 0 is default (return 1 for 'no tail')
    ...
    ,
    effGetParameterProperties ///< [index]: parameter index [ptr]:
                              ///< #VstParameterProperties* [return value]: 1 if supported
    ...
    effEditKeyDown ///< [index]: ASCII character [value]: virtual key [opt]: modifiers
                   ///< [return value]: 1 if key used  @see AEffEditor::onKeyDown
    ,
    effEditKeyUp ///< [index]: ASCII character [value]: virtual key [opt]: modifiers
                 ///< [return value]: 1 if key used  @see AEffEditor::onKeyUp
    ,
    effSetEditKnobMode ///< [value]: knob mode 0: circular, 1: circular relativ, 2: linear
                       ///< (CKnobMode in VSTGUI)  @see AEffEditor::setKnobMode

    ,
    effGetMidiProgramName ///< [index]: MIDI channel [ptr]: #MidiProgramName* [return
                          ///< value]: number of used programs, 0 if unsupported  @see
                          ///< AudioEffectX::getMidiProgramName
    ,
    effGetCurrentMidiProgram ///< [index]: MIDI channel [ptr]: #MidiProgramName* [return
                             ///< value]: index of current program  @see
                             ///< AudioEffectX::getCurrentMidiProgram
    ,
    effGetMidiProgramCategory ///< [index]: MIDI channel [ptr]: #MidiProgramCategory*
                              ///< [return value]: number of used categories, 0 if
                              ///< unsupported  @see AudioEffectX::getMidiProgramCategory
    ,
    effHasMidiProgramsChanged ///< [index]: MIDI channel [return value]: 1 if the
                              ///< #MidiProgramName(s) or #MidiKeyName(s) have changed
                              ///< @see AudioEffectX::hasMidiProgramsChanged
    ,
    effGetMidiKeyName ///< [index]: MIDI channel [ptr]: #MidiKeyName* [return value]: true
                      ///< if supported, false otherwise  @see
    ...
    ,
    effGetSpeakerArrangement ///< [value]: input #VstSpeakerArrangement* [ptr]: output
                             ///< #VstSpeakerArrangement*  @see
                             ///< AudioEffectX::getSpeakerArrangement
    ,
    effShellGetNextPlugin ///< [ptr]: buffer for plug-in name, limited to
                          ///< #kVstMaxProductStrLen [return value]: next plugin's
                          ///< uniqueID  @see AudioEffectX::getNextShellPlugin
    ...
    effSetTotalSampleToProcess ///< [value]: number of samples to process, offline only!
                               ///< @see AudioEffectX::setTotalSampleToProcess
    ,
    effSetPanLaw ///< [value]: pan law [opt]: gain  @see VstPanLawType @see
                 ///< AudioEffectX::setPanLaw

    ,
    effBeginLoadBank ///< [ptr]: #VstPatchChunkInfo* [return value]: -1: bank can't be
                     ///< loaded, 1: bank can be loaded, 0: unsupported  @see
                     ///< AudioEffectX::beginLoadBank
    ,
    effBeginLoadProgram ///< [ptr]: #VstPatchChunkInfo* [return value]: -1: prog can't be
                        ///< loaded, 1: prog can be loaded, 0: unsupported  @see
                        ///< AudioEffectX::beginLoadProgram
#endif // VST_2_3_EXTENSIONS

#if VST_2_4_EXTENSIONS
    ,
    effSetProcessPrecision ///< [value]: @see VstProcessPrecision  @see
                           ///< AudioEffectX::setProcessPrecision
    ,
    effGetNumMidiInputChannels ///< [return value]: number of used MIDI input channels
                               ///< (1-15)  @see AudioEffectX::getNumMidiInputChannels
    ,
    effGetNumMidiOutputChannels ///< [return value]: number of used MIDI output channels
                                ///< (1-15)  @see AudioEffectX::getNumMidiOutputChannels
...
struct VstParameterProperties {
    ...
    VstInt16 displayIndex; ///< index where this parameter should be displayed (starting
                           ///< with 0)
...
enum VstParameterFlags {
    ...
    kVstParameterUsesFloatStep = 1
        << 2, ///< stepFloat, smallStepFloat, largeStepFloat valid
...
enum VstPinPropertiesFlags {
    ...
    kVstPinUseSpeaker = 1 << 2 ///< #VstPinProperties::arrangementType is valid and can be
                               ///< used to get the wanted arrangement
    //-------------------------------------------------------------------------------------------------------
};
...
enum VstPlugCategory {
    ...
    kPlugCategShell, ///< Plug-in is container of other plug-ins  @see
...
enum VstMidiProgramNameFlags {
    //-------------------------------------------------------------------------------------------------------
    kMidiIsOmni = 1 ///< default is multi. for omni mode, channel 0 is used for inquiries
                    ///< and program changes
    //-------------------------------------------------------------------------------------------------------
};
...
struct VstSpeakerProperties {
    ...
    char name[kVstMaxNameLen]; ///< for new setups, new names should be given (L/R/C...
                               ///< won't do)
...
struct VstFileType {
    ...
    VstFileType(const char* _name = 0, const char* _macType = 0, const char* _dosType = 0,
        const char* _unixType = 0, const char* _mimeType1 = 0,
...
struct VstFileSelect {
    ...
    char* returnPath; ///< use with #kVstFileLoad and #kVstDirectorySelect. null: Host
                      ///< allocates memory, plug-in must call #closeOpenFileSelector!
    VstInt32 sizeReturnPath; ///< size of allocated memory for return paths
    char** returnMultiplePaths; ///< use with kVstMultipleFilesLoad. Host allocates
                                ///< memory, plug-in must call #closeOpenFileSelector!
